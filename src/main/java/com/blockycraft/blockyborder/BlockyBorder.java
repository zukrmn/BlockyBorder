...
public class BlockyBorder extends JavaPlugin {
  ...
  public void onEnable() {
    ...
    getServer().getScheduler().scheduleSyncRepeatingTask((Plugin)this, new Runnable() {
          public void run() {
            BlockyBorder.this.borderIgnoreTick();
          }
        },  1L, 1L);
  }
  ...
  private class BorderPlayerListener extends PlayerListener {
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
      if (!BlockyBorder.this.enabled)
        return; 
      Player player = event.getPlayer();
      if (BlockyBorder.this.ignoreBorderTicks.containsKey(player.getUniqueId()))
        return; 
      ...
      if (toX < BlockyBorder.this.minX || toX > BlockyBorder.this.maxX || toZ < BlockyBorder.this.minZ || toZ > BlockyBorder.this.maxZ)
        if (BlockyBorder.this.loopEnabled) {
          ...
          if (toX < BlockyBorder.this.minX) {
            newLoc.setX(BlockyBorder.this.maxX - BlockyBorder.this.buffer);
          } else if (toX > BlockyBorder.this.maxX) {
            newLoc.setX(BlockyBorder.this.minX + BlockyBorder.this.buffer);
          } 
          if (toZ < BlockyBorder.this.minZ) {
            newLoc.setZ(BlockyBorder.this.maxZ - BlockyBorder.this.buffer);
          } else if (toZ > BlockyBorder.this.maxZ) {
            newLoc.setZ(BlockyBorder.this.minZ + BlockyBorder.this.buffer);
          } 
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    ...
    if (args.length >= 1)
      try {
        freq = Math.max(1, Integer.parseInt(args[0]));
      } catch (Exception exception) {} 
    if (args.length >= 2)
      try {
        pad = Integer.parseInt(args[1]);
      } catch (Exception exception) {} 
    sender.sendMessage("[BlockyBorder] Prdos chunks iniciada...");
    ...
    scheduler.scheduleSyncRepeatingTask((Plugin)this, new FillTask(world, cminX, cmaxX, cminZ, cmaxZ, freq, sender), 0L, 1L);
    return true;
  }
  ...
  class FillTask implements Runnable {
    ...
    public void run() {
      int count = 0;
      while (count < this.freq && this.curX <= this.maxX) {
      ...
      if (this.done % 100 == 0)
        this.sender.sendMessage("[BlockyBorder] Chunks gerados: " + this.done + "/" + this.total); 
      if (this.curX > this.maxX)
        this.sender.sendMessage("[BlockyBorder] PrconcluChunks gerados: " + this.done); 
    }
  }
}